...
const buttonsStyle = (width: number, height: number, button: any, x: number, y: number) => {
...
const defaultState: IGameContextProps = {
    ...
    setMode: (ctx: p5.Renderer, width: number, height: number, buttons: any) => {
        buttonsStyle(width, height, buttons.online, ctx.position().x + width * 0.25, ctx.position().y + height * 0.38);
        buttonsStyle(width, height, buttons.offline, ctx.position().x + width * 0.25, ctx.position().y + height * 0.52);
    ...
    setNumberOfPlayers: (ctx: p5.Renderer, width: number, height: number,
        buttons: any) => {
        buttonsStyle(width, height, buttons.oneplayer, ctx.position().x + width * 0.25, ctx.position().y + height * 0.38);
        buttonsStyle(width, height, buttons.twoplayer, ctx.position().x + width * 0.25, ctx.position().y + height * 0.52);
    ...
    setPlayerTool: (ctx: p5.Renderer, width: number, height: number, buttons: any) => {
        buttonsStyle(width, height, buttons.mouse, ctx.position().x + width * 0.25, ctx.position().y + height * 0.38);
        buttonsStyle(width, height, buttons.keybord, ctx.position().x + width * 0.25, ctx.position().y + height * 0.52);
    ...
    setDifficulty: (ctx: p5.Renderer, width: number, height: number, buttons: any) => {
        buttonsStyle(width, height, buttons.easy, ctx.position().x + width * 0.25, ctx.position().y + height * 0.31);
        buttonsStyle(width, height, buttons.medium, ctx.position().x + width * 0.25, ctx.position().y + height * 0.45);
        buttonsStyle(width, height, buttons.hard, ctx.position().x + width * 0.25, ctx.position().y + height * 0.59);
...
export default function Game(props: any) {
    ...
    const draw = (p5: p5Types) => {
        ...
        if (!gameState.GameLoaded) {
            if (gameState.mode === 'offline' && gameState.difficulty === '' && gameState.numberOfPlayers === 1
                && gameState.playerTool !== '') {
                gameState.setDifficulty(ctx, props.width, props.height, gameState.buttons);
            }
            else if ((gameState.mode === 'online' || gameState.numberOfPlayers === 1) && gameState.playerTool === '') {
            ...
            if (!gameState.pongData.isGameStarted) {
                ...
                if (gameState.mode === 'online') {
                    ...
                    if (gameState.playerTool === 'mouse' && p5.mouseY > 0 && p5.mouseY < props.height - props.height / 4) {
                ...
                p5.fill(179, 240, 255);
                p5.circle(gameState.pongData.ball.x * props.width, gameState.pongData.ball.y * props.height, gameState.radius);
                p5.fill(102, 181, 255);
                p5.rect(0, gameState.pongData.playerLeft.position * props.height, props.width / 60, props.height / 4);

                p5.fill(77, 77, 255);
                p5.rect(props.width - props.width / 60, gameState.pongData.playerRight.position * props.height, props.width / 60, props.height / 4);
                if (!gameState.pongData.isGameStarted) {
                    p5.noLoop();
                }
